...
import { getTechCrunch } from './sources/techcrunch.js';
...
export async function getDashboard(force: boolean): Promise<DashboardData> {
  const [server, indices, holdings, techcrunch, github, kr] = await Promise.all([
    ...
    source('techcrunch', config.ttl.techcrunch, force, getTechCrunch, []),
